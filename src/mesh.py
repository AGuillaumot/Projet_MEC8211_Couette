...
class Mesh:
    ...
    def get_face_to_norm(self, face):
        """
        Retourne la norme de l'arrête spécifiée.
    
        Parameters
        ----------
        face: int
            Index de l'arête que l'on souhaite récupérer sa normale.
        ...
        """
        nodes = self.get_face_to_nodes(face)
         
        nodes_coord = [np.array(self.get_node_to_xycoord(nodes[0])), np.array(self.get_node_to_xycoord(nodes[1]))]
        
        norm = np.linalg.norm(nodes_coord[1]-nodes_coord[0])
        return norm
    
    def get_face_to_normal(self, face):
        """
        Retourne la projection selon x et y de la normale à l'arête spécifiée.
        ...
        -------
        double
            Projection selon x de la normale.
        double
            Projection selon y de la normale.
        """
        nodes = self.get_face_to_nodes(face)
         
        nodes_coord = [self.get_node_to_xycoord(nodes[0]), self.get_node_to_xycoord(nodes[1])]
            
        normal = np.array([(nodes_coord[1][1] - nodes_coord[0][1]), -(nodes_coord[1][0] - nodes_coord[0][0])])
    ...
    def get_nodes_to_xy_mean_coord(self, nodes):
        """
        Retourne les coordonnées moyennes x et y des noeuds spécifiés.
    
        Parameters
        ----------
        nodes: ndrray
            Index des noeuds que l'on souhaite récupérer les coordonnées.
    
        Returns
        -------
        double
            Coordonnées x du noeud central fictif.
        double
            Coordonnée y du noeud central fictif.
    
        """
        xy_coord = np.zeros((len(nodes),2))
        
        for k in range(len(nodes)) :
            xy_coord[k] = self.get_node_to_xycoord(nodes[k])
            
        xy_mean_coord = np.mean(xy_coord, axis=0)
        return xy_mean_coord
    
    def elements_coords (self):
        """
        Retourne les coordonnées  x et y des centres des éléments du maillage
        ...
        -------
        ndarray
            Coordonnées moyennes x et y des centres des éléments du maillage.
    
        """
        n_elements = self.get_number_of_elements()
        elements_coords = np.zeros((n_elements, 2))       # Initialisation de la matrice contenant la position du centre de chaque 
        for i_element in range(n_elements):             # Boucle pour obtenir la position du centre de chaque élément 
            nodes = self.get_element_to_nodes(i_element)
            elements_coords [i_element] = self.get_nodes_to_xy_mean_coord(nodes)
    ...
    def faces_coords (self):
        ...
        """
        n_faces = self.get_number_of_faces()
        faces_coords = np.zeros((n_faces, 2))       # Initialisation de la matrice contenant la position du centre de chaque arête
        for i_face in range(n_faces):             # Boucle pour obtenir la position du centre de chaque Arête 
            nodes = self.get_face_to_nodes(i_face)
            faces_coords [i_face] = self.get_nodes_to_xy_mean_coord(nodes)
        return faces_coords
    
    def elements_area (self):
        """
        Retourne les aires des éléments du maillage.
        ...
        -------
        ndarray
            Aires des éléments du maillage.
    
        """
        n_elements = self.get_number_of_elements()
        aire = np.zeros(n_elements)
        for i_element in range(n_elements):
            nodes = self.get_element_to_nodes(i_element)
            x_coords = np.zeros(len(nodes))
            y_coords = np.zeros(len(nodes))
            for k_node in range(len(nodes)):
                x_coords[k_node] = self.get_node_to_xcoord(nodes[k_node])
                y_coords[k_node] = self.get_node_to_ycoord(nodes[k_node])
            
            sum1 = np.dot(x_coords, np.roll(y_coords, -1))
            sum2 = np.dot(y_coords, np.roll(x_coords, -1))
            
            aire[i_element] = 0.5 * np.abs(sum1 - sum2)
        return aire