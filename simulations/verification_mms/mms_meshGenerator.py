...
class MeshGenerator:
    ...
    def rectangle(self, rectangle_corners, mesh_parameters):
        ...
        return mesh_obj
    def rectangle_perturbation(self, rectangle_corners, mesh_parameters):
        gmsh.initialize()
        gmsh.option.setNumber("General.Terminal", self.verbose)
        gmsh.model.add("Rectangle")
    
        # Taille caractéristique ou 0 si transfinite
        lc = mesh_parameters.get('lc', 0.0)
    
        dx = rectangle_corners[1] - rectangle_corners[0]
        dy = rectangle_corners[3] - rectangle_corners[2]
        ...
        gmsh.model.geo.addPoint(rectangle_corners[0], rectangle_corners[2], 0.0, lc, 3)  # Pt3
        gmsh.model.geo.addPoint(rectangle_corners[1], rectangle_corners[2], 0.0, lc, 4)  # Pt4
        gmsh.model.geo.addPoint(rectangle_corners[0]+dx*0.2, rectangle_corners[2]+dy*0.2, 0.0, lc, 5)  # Pt5
        gmsh.model.geo.addPoint(rectangle_corners[0]+dx*0.2+dx*0.1, rectangle_corners[2]+dy*0.2, 0.0, lc, 6)  # Pt6
        gmsh.model.geo.addPoint(rectangle_corners[0]+dx*0.2, rectangle_corners[2], 0.0, lc, 7)  # Pt7
        gmsh.model.geo.addPoint(rectangle_corners[0]+dx*0.2+dx*0.1, rectangle_corners[2], 0.0, lc, 8)  # Pt8
        gmsh.model.geo.addPoint(rectangle_corners[0]+dx*0.2+dx*0.1, rectangle_corners[3], 0.0, lc, 9)  # Pt9
        gmsh.model.geo.addPoint(rectangle_corners[0]+dx*0.2, rectangle_corners[3], 0.0, lc, 10)  # Pt10
        ...
        gmsh.model.geo.addLine(10, 2, 10)
        gmsh.model.geo.addLine(6, 10, 11)
        gmsh.model.geo.addLine(5, 9, 12)
    
        # --- Curve loops & surfaces
        gmsh.model.geo.addCurveLoop([10, 2, 3, 5, 11], 1)
        gmsh.model.geo.addCurveLoop([9, -11, 6, 12], 2)
        gmsh.model.geo.addCurveLoop([1, -12, 7, 8, 4], 3)
        gmsh.model.geo.addPlaneSurface([1], 1)
        gmsh.model.geo.addPlaneSurface([2], 2)
        gmsh.model.geo.addPlaneSurface([3], 3)
        gmsh.model.addPhysicalGroup(2, [1, 2, 3], 1)
        gmsh.model.setPhysicalName(2, 1, "DOMAINE")
        ...
        if 'lc' not in mesh_parameters:
            ...
            nx3 = max(int(nx * 0.2), 2)
            nx6 = max(int(nx * 0.1), 2)
            nx4 = nx - nx3 - nx6  # pour que somme = nx
    
            ny1 = max(int(ny * 0.2), 2)
            ny2 = ny - ny1
    
            gmsh.model.geo.mesh.setTransfiniteCurve(3, nx3)
            gmsh.model.geo.mesh.setTransfiniteCurve(10, nx3)
            gmsh.model.geo.mesh.setTransfiniteCurve(6, nx6)
            gmsh.model.geo.mesh.setTransfiniteCurve(9, nx6)
            gmsh.model.geo.mesh.setTransfiniteCurve(1, nx4)
            gmsh.model.geo.mesh.setTransfiniteCurve(8, nx4)
    
            gmsh.model.geo.mesh.setTransfiniteCurve(2, ny)
            gmsh.model.geo.mesh.setTransfiniteCurve(4, ny)
            gmsh.model.geo.mesh.setTransfiniteCurve(5, ny1)
            gmsh.model.geo.mesh.setTransfiniteCurve(7, ny1)
            gmsh.model.geo.mesh.setTransfiniteCurve(11, ny2)
            gmsh.model.geo.mesh.setTransfiniteCurve(12, ny2)
    
            gmsh.model.geo.mesh.setTransfiniteSurface(1, "Alternate")
            gmsh.model.geo.mesh.setTransfiniteSurface(2, "Alternate")
            gmsh.model.geo.mesh.setTransfiniteSurface(3, "Right", cornerTags=[1, 9, 8, 4])
        ...
        if not elem_node_tags_domaine or all(len(e) == 0 for e in elem_node_tags_domaine):
            gmsh.write("debug_failed_mesh.msh")
            gmsh.finalize()
            raise RuntimeError("Échec du maillage — aucun élément 2D généré.")
    
        # --- Bords (tags géométriques des courbes)
        boundary_tags = [[2], [3, 5, 6, 7, 8], [4], [1, 9, 10]]
        elem_boundary = []
        for b_tags in boundary_tags:
            elem_bound = []
            for b_tag in b_tags:
            ...
            if elem_bound:
                ...
                elem_bound = np.array([], dtype=int)
            elem_boundary.append(elem_bound)
    
        gmsh.finalize()
    
        return self.generate_mesh_object(node_coords, elem_types_domaine, elem_node_tags_domaine, elem_boundary)
    ...
    def back_step(self, H1, H2, L1, L2, mesh_parameters):
        ...
        gmsh.model.geo.addPoint(L2/2,  H1-H2,  0.0, lc, 8)  
        gmsh.model.geo.addPoint(L2/2,  H1,     0.0, lc, 9)  
        ...
        gmsh.model.geo.addLine(9, 8, 13) 
        
        boundary_tags = [[1], [2, 3, 9, 10], [5], [11,12, 7]]
        # # Surface zone and Physicals Group
        gmsh.model.geo.addCurveLoop([1, 2, -8, 7], 1)
        gmsh.model.geo.addCurveLoop([8, 3, 9, -13, 12], 2)
        gmsh.model.geo.addCurveLoop([13, 10, 5, 11], 3)
        ...
        if 'lc' not in mesh_parameters:
            nx = mesh_parameters['Nx']+1
            nx1 = max(int(L1*nx/L2*0.5), 2)
            nx2 = max(int(0.5*0.5*nx), 2)
            nx3 = max(nx - nx1 - nx2, 2)
            nx = nx1 + nx2+ nx3
            ny = mesh_parameters['Ny']+1
            ...
            gmsh.model.geo.mesh.setTransfiniteCurve(9, nx3)
            gmsh.model.geo.mesh.setTransfiniteCurve(10, nx2)
            
            gmsh.model.geo.mesh.setTransfiniteCurve(5, ny)
            
            gmsh.model.geo.mesh.setTransfiniteCurve(11, nx2)
            gmsh.model.geo.mesh.setTransfiniteCurve(12, nx3)
            ...
            gmsh.model.geo.mesh.setTransfiniteSurface(1, "Alternate")
            gmsh.model.geo.mesh.setTransfiniteSurface(2, "Right", cornerTags=[7, 4, 8, 9])
